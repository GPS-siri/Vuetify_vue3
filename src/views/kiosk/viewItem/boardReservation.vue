<template>
	<div>
		<div class="wrapper">
			...
			<v-layout align-center class="header_search">
				<v-layout class="search_wrap">
					<v-layout>
						...
						<v-flex class="search_select_type ml-2" style="width:300px; max-width:300px;">
							<selectBox
								:sel="surveySel"
								class="searchSel"
								@change="
									() => {
										this.detail_dialog.surveySel = surveySel.value
										first = true
			...
			<v-layout align-center class="filter_search mt-4" style="min-height:50px !important; height:auto !important; padding: 4px 0px;">
				<v-flex xs12 class="px-4">
					<v-layout wrap>
						<v-flex xs4 v-for="(filter, index) in table.filter2" :key="index" class="my-4">
							<v-layout align-center>
								...
								<v-layout xs12 v-else-if="filter.date" class="search_select_type" style="margin-left:10px; width:100%;">
									<v-flex :class="$vuetify.breakpoint.smAndDown ? 'my-2' : 'search_date_1'">
										<DatepickerDialog :picker="filter.date.start"></DatepickerDialog>
									</v-flex>
									<span class="mx-1"> ~ </span>
									<v-flex :class="$vuetify.breakpoint.smAndDown ? 'my-2 ' : 'search_date_2'">
										<DatepickerDialog :picker="filter.date.end" :allowed_dates="termCustomer"> </DatepickerDialog>
									</v-flex>
								</v-layout>
						...
						<v-flex xs4 class="mt-4">
							<v-layout align-center>
								<div class="customer-management-text" style="min-width:80px;max-width:200px; width:auto; text-align:center;">
									방문여부
								</div>
								<div xs12 class="search_select_type" style="margin-left:10px; width:100%;max-width:200px;">
									<selectBox :sel="visit" class="searchSel"></selectBox>
								</div>
							</v-layout>
						</v-flex>
						<v-layout align-center justify-end class="my-1">
							<v-btn class="customer-management-btn" @click="addFilter()">
								필터적용
							</v-btn>
						</v-layout>
					</v-layout>
			...
			<v-layout wrap>
				...
				<v-flex xs12>
					<datatable
						...
						excelUseYn="true"
						excelTextYn="true"
						excelType="reservation_customer"
					>
					</datatable>
				</v-flex>
			</v-layout>
		...
		<campaignCustomer :setdialog="detail_dialog" type="day" />
		<!-- 사용자 생성/수정 alert ------ end -->
	</div>
</template>
	...
	data() {
		return {
			...
			table: {
				datatable: {
					headers: [
						{ text: '방문예약명', value: 'title', width: '20%' },
						{ text: '고객명', value: 'name', width: '13%' },
						{ text: '휴대전화번호', value: 'phone', width: '13%' },
						{ text: '예약일시', value: 'rdate', width: '13%' },
						{ text: '방문일시', value: 'updated_at_time', width: '13%' },
						{ text: '방문여부', value: 'state_visit', width: '13%' },
						{ text: '취소일시', value: 'updated_at_time', width: '13%' },
						{
							text: '비고',
							align: 'center',
							value: 'etc_detail',
	...
	methods: {
		...
		async getTableData(val) {
			...
			headers = [
				...
				{ text: '휴대전화번호', value: 'phone', width: '13%' },
				{ text: '예약일시', value: 'rdate', width: '13%' },
				{ text: '방문일시', value: 'state_visit_date', width: '13%' },
				{ text: '방문여부', value: 'state_visit', width: '13%' },
				{ text: '취소일시', value: 'cancel_visit', width: '13%' },
				{
					text: '비고',
					align: 'center',
					value: 'etc_detail',
			...
			await this.$store.dispatch('customer_campagin_reserveusers', data).then(res => {
				...
				if (val.input_box) {
					...
					this.table = JSON.parse(JSON.stringify(tableItems))
					this.originTable = JSON.parse(JSON.stringify(tableItems))
				} else {
					tableItems = {
						datatable: {
							headers: headers,
			...
			await this.$store.dispatch('businesses', input).then(resData => {
				if (resData.data.businesses.length > 0) {
					...
					this.$store.dispatch('reservedays', data).then(res => {
						...
						this.detail_dialog.surveySel = this.surveySel.value
		...
		addFilter() {
			...
			// 				if (item.status !== 2) {
			// 					if (this.visit.value === '미방문') {
			// 						if (item.state === 'reserve') return true
			// 						else {
			// 							if (this.searchsel_city1.value && this.searchsel_city2.value && this.searchsel_city3.value) {
			// 								if (
			// 									item.locationData ===
			...
			// 						}
			// 					} else {
			// 						if (item.state === 'visit') return true
			// 						else {
			// 							if (this.searchsel_city1.value && this.searchsel_city2.value && this.searchsel_city3.value) {
			// 								if (
			// 									item.locationData ===
			...
			// 			} else {
			// 				if (this.searchsel_city1.value && this.searchsel_city2.value && this.searchsel_city3.value) {
			// 					if (item.locationData === this.searchsel_city1.value + '_' + this.searchsel_city2.value + '_' + this.searchsel_city3.value)
			// 						return true
			// 				} else {
			// 					return true
			// 				}
			...
			// 	}
			// })
			let result = this.originTable.datatable.items
			result = result.filter(
				x => x.updated_at && this.table.filter2[0].date.start.date < x.updated_at && this.table.filter2[0].date.end.date > x.updated_at,
			)
			if (this.visit.value !== '전체' && this.visit.value !== null) {
				if (this.visit.value === '취소') {
					result = result.filter(x => x.status === 2)
				} else {
					result = result.filter(x => x.status === 1 && x.state === (this.visit.value === '방문' ? 'visit' : 'reserve'))
				}
			}

			this.table.datatable.items = result
		},
