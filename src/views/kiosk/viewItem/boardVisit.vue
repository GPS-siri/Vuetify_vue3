<template>
	<div>
		...
		<campaignCustomer :setdialog="detail_dialog" type="visit" />
		<!-- 사용자 생성/수정 alert ------ end -->
	</div>
</template>
	...
	methods: {
		...
		getTableData(val) {
			...
			headers = [
				...
				{ text: '고객명', value: 'name', width: '13%' },
				{ text: '휴대전화번호', value: 'phone', width: '13%' },
				{ text: '방문일시', value: 'created_at_time', width: '13%' },
				// { text: '방문형태', value: 'visit', width: '13%' },
				{
					text: '비고',
					align: 'center',
					value: 'etc_detail',
			...
			datas.type = 'd'
			datas.reserveType = 'day'
			data.type = 'd'
			this.$store.dispatch('reserveusersConnection', datas).then(count => {
				if (count.data.reserveusersConnection.aggregate.count !== 0) data.reserveType_nin = ['day']
				this.$store.dispatch('customer_campagin_reserveusers', data).then(res => {
