<template>
	<div>
		<div class="wrapper">
			...
			<v-layout align-center class="header_search">
				<v-layout class="search_wrap">
					<v-layout>
						...
						<v-flex class="search_select_type ml-4" style="width:300px; max-width:300px;">
							<selectBox :sel="surveySel" class="searchSel " @change="getTableData"></selectBox>
						</v-flex>
					</v-layout>
					<v-layout justify-end>
						...
						<v-btn class="search_btn" color="#5d19ff" @click="customer_waiting_reserveusers_call()"><v-icon>mdi-magnify</v-icon>조회</v-btn>
						<v-btn
							class="search_btn"
							color="#5d19ff"
							@click="
			...
			<v-layout align-center class="filter_search3 mt-4" style="min-height:50px !important; height:auto !important; padding: 20px 0px;">
				<v-flex xs12 class="px-4">
					<v-layout wrap>
						<v-flex xs4 v-for="(filter, index) in table.filter" :key="index" class="mt-4">
							<v-layout align-center>
								<div class="customer-management-text" style="min-width:80px; width:auto; text-align:center;">
									{{ filter.check.label }}
								</div>
						...
						<v-flex xs4 class="mt-4">
							<v-layout align-center>
								...
								<div xs12 class="search_select_type" style="margin-left:10px; width:100%;">
									<selectBox :sel="like" class="searchSel"></selectBox>
								</div>
							</v-layout>
						</v-flex>
						<v-flex xs4 class="mt-4">
							<v-layout align-center>
								...
								<div xs12 class="search_select_type" style="margin-left:10px; width:100%;">
									<selectBox :sel="counselorFilter" class="searchSel"></selectBox>
								</div>
							</v-layout>
						</v-flex>
						<v-flex xs4 class="mt-4">
							<v-layout align-center>
								...
								<div xs12 class="search_select_type" style="margin-left:10px; width:100%;">
									<selectBox :sel="visit" class="searchSel"></selectBox>
								</div>
							</v-layout>
						</v-flex>
					</v-layout>
					<v-layout align-center justify-end class="my-1">
		...
		<button id="refresh-coun" @click="refresh" style="display:none;"></button>
		<!-- 사용자 생성/수정 alert ------ start -->
		<counselorCustomer :setdialog="detail_dialog" />
		<!-- 사용자 생성/수정 alert ------ end -->
	</div>
</template>

<script>
import { datatable, selectBox, txtField, DatepickerDialog, counselorCustomer } from '@/components/index.js'
import location1 from '../../../utils/location1'
import location2 from '../../../utils/location2'
export default {
	components: { datatable, selectBox, txtField, DatepickerDialog, counselorCustomer },
	data() {
		return {
			tableData: [],
			businessId: '',
			...
			table: {
				datatable: {
					headers: [
						{ text: '방문등록명', value: 'title', width: '20%' },
						{ text: '고객명', value: 'name' },
						{ text: '휴대전화번호', value: 'phone' },
						{ text: '상담 신청 일시', value: 'create_at', width: '13%' },
						{ text: '상태', value: 'stat' },
						{ text: '처리 일시', value: 'rdate', width: '13%' },
	...
	methods: {
		...
		async customer_waiting_counselor_reserveusers_call() {
			this.$store.dispatch('customer_waiting_counselor_reserveusers', {}).then(res => {
				let obj = {}
				for (let i = 0; i < res.data.counselors.length; i++) {
					obj[res.data.counselors[i].counselorid] = res.data.counselors[i].name
				}
				let items = []
				...
				this.counselors = obj
				this.detail_dialog.counselors = obj
				this.detail_dialog.counselorsList = items
			})
		},
		async getTableData(val) {
			...
			let headers = [
				...
				{ text: '고객명', value: 'name' },
				{ text: '휴대전화번호', value: 'phone' },
				{ text: '상담 신청 일시', value: 'created_at', width: '13%' },
				{ text: '상태', value: 'stat' },
				{ text: '처리 일시', value: 'rdate', width: '13%' },
				{ text: '상담사', value: 'counselorid' },
				{ text: '호응도', value: 'custlike' },
			...
			await this.$store.dispatch('waitings', data).then(res => {
				this.waitingsItem = res.data.waitings.filter(e => {
					return e.stat !== 99
				})

				this.waitingsItem.forEach(item => {
					if (!item.userData) {
						this.userData_setting(item)
					}
				})
				...
				if (val.input_box) {
					...
					this.tableData = JSON.parse(JSON.stringify(this.waitingsItem))
		...
		getCounselors() {
			if (this.surveySel.value === null) {
				return (this.$store.state.loading = false)
			} else if (this.surveySel.value.business.id) {
				const data = {
					business: this.surveySel.value.business.id,
				}
				this.$store
					...
					.then(res => {
						...
						this.counselorFilter.items = items
					})
					.catch(() => {})
			} else {
				this.counselorFilter.items = ['전체']
			}
		},
		changeTypeText(data) {
			if (data === 11) {
		...
		customer_waiting_reserveusers_call() {
			this.first = false
			this.getTableData(this.surveySel.value)
		},
		detailClick(item) {
			...
			this.detail_dialog.reserveday = this.surveySel.value
			this.detail_dialog.counselorsList = this.counselorFilter.items
		...
		userData_setting(item) {
			item.rdate = this.$moment(item.rdate)._d
			if (item.updated_at) {
				item.updated_at = this.$moment(item.updated_at).format('YYYY-MM-DD HH:mm:ss')
			}
			item['categories'] = JSON.parse(JSON.stringify(this.surveySel.value.input_box.categories))
			let userData = []
			item.categories.forEach(a => {
				if (a.stat === 1) {
					if (a.type === 31 || a.type === 30) {
						// type 날짜일 경우 예약일시 적용
					} else {
						if (a.type !== 21) {
						...
						if (a.type === 11) {
							item[this.changeTypeText(a.type)].split('/').forEach(type => {
								if (type.split(':')[0] === a.code) {
									...
									item[type.split(':')[0]] = checkItem.toString()

									a['qusType'] = '객관식(다중선택)'
									a['value'] = item[type.split(':')[0]]
									userData.push(a)
								}
							})
						} else if (a.type === 10) {
							item[this.changeTypeText(a.type)].split('/').forEach(type => {
								if (type.split(':')[0] === a.code && type.split(':')[1]) {
									...
									if (a.category_details[valIndex] !== undefined) {
										...
										if (value === '기타') {
											if (item.etc) {
												let etc = item.etc.split('/')
												let etcIndex = etc.findIndex(e => e.indexOf(type.split(':')[1]) !== -1)
												if (etc[etcIndex]) {
													if (etc[etcIndex].split(':')[1] !== '') {
														item[type.split(':')[0]] = `기타: ${etc[etcIndex].split(':')[1]}`

														a['qusType'] = '객관식(단일선택)'
														a['value'] = item[type.split(':')[0]]
														userData.push(a)
													} else {
														item[type.split(':')[0]] = '기타:'
							...
							item[this.changeTypeText(a.type)].split('/').forEach(type => {
								if (type.split(':')[0] === a.code) {
									item[type.split(':')[0]] = type.split(':')[1]
		...
		addFilter(table) {
			...
			table.filter.forEach((e, index) => {
				...
				if (index === table.filter.length - 1) {
					result = result.filter(item => {
						if (this.visit.value !== '전체') {
							if (this.visit.value === '동의') return item.ispersonal === 1
							else return item.ispersonal === 2
						} else return true
					})
					result = result.filter(item => {
						if (this.like.value !== '전체') {
							if (item.custlike === this.like.value) return true
							else return false
						} else return true
					})
					result = result.filter(item => {
						if (this.counselorFilter.value !== '전체') {
							if (item.counselorid === this.counselorFilter.value) return true
							else return false
						} else return true
					})
				}
