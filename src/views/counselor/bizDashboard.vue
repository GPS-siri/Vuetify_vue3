	...
	<v-container class="login-counselor counselor_gray_background mx-auto" style="max-width:100vw; width:100vw !important">
		...
		<v-layout style=" margin: 0 auto;" wrap justify-center>
			...
			<div class="dash_counselor">
				<!-- 상담사 정보 / qr -->
				<v-layout wrap align-center class="dash_counselor_info">
					<v-flex xs9>
						<div>
					...
					<v-flex xs3 class="display_flex">
						<v-btn
							color="primary2"
							class="dash_business_btn"
							style="border-radius:10px;"
							@click="click_move_work"
							block
							dark
							depressed
							height="40"
				...
				<!-- 활동 현황 -->
				<v-layout wrap align-center class="dash_counselor_performance">
					<v-flex xs8 class="performance_title" style="height:40px">
						<span>
							활동현황
						</span>
					</v-flex>
					<v-flex xs4 style="padding-bottom:12px; border-bottom: 1px solid rgba(187, 187, 187, 0.5);">
						<v-layout class="dash_detail" @click="dashboard_detail = !dashboard_detail">
							<v-img class="dash_img" src="@/assets/images/ico/ico_trending-up.png" style=""></v-img>
							<span style="font-size: 14px; font-weight: normal; color:#9AA6AC">
								자세히 보기
							</span>
						</v-layout>
					</v-flex>
			...
			<!-- 자세히 보기 -->
			<v-layout wrap v-if="dashboard_detail" class="px-5" style="margin-bottom: 70px;">
				<v-flex xs12 class="dashboard_detail_main mt-3 pt-1">
					<v-layout class="my-2">
					...
					<v-layout class="mt-1">
						<v-flex xs3 class="mr-2">
							<select-box class="select_box_ico" :sel="detailData.goodsCustomer.select_1"></select-box>
						</v-flex>
						<v-flex xs3 class="mr-2">
							<select-box class="select_box_ico" :sel="detailData.goodsCustomer.select_2"></select-box>
						</v-flex>
						<v-flex xs3 class="mr-2">
							<select-box class="select_box_ico" :sel="detailData.goodsCustomer.select_3"></select-box>
						</v-flex>
						<v-flex xs3>
							<v-btn color="primary2" class="dash_business_btn" style="border-radius:5px;" block dark depressed height="25" width="69">
								적용
							</v-btn>
						</v-flex>
					</v-layout>
					<v-layout class="mb-2">
						<v-data-table
							...
							height="130px"
							:headers="detailData.goodsCustomer.table.headers"
							:items="detailData.goodsCustomer.table.items"
							mobile-breakpoint="0"
							:disable-sort="false"
							:fixed-header="true"
				...
				<v-flex xs12 class="dashboard_detail_main mt-3 pt-1">
					...
					<v-layout class="mt-1 mb-2">
						<v-data-table
							class="dashboard_detail_table"
							height="130px"
							:headers="detailData.statusCustomer.table.headers"
							:items="detailData.statusCustomer.table.items"
							mobile-breakpoint="0"
							:disable-sort="false"
							:fixed-header="true"
							:hide-default-footer="true"
				...
				<v-flex xs12 class="dashboard_detail_main mt-3 pt-1">
					<v-layout class="my-2">
						<v-flex class="mr-auto" style="font-size:12px; ">
							알림톡 발송 현황
						</v-flex>
						<v-flex xs3 class="mr-2">
							<select-box class="select_box_ico" :sel="detailData.talk.select_1"></select-box>
						</v-flex>
						<v-flex xs3>
							<v-btn color="primary2" class="dash_business_btn" style="border-radius:5px;" block dark depressed height="25" width="69">
								적용
					...
					<v-layout class="mt-1 mb-2">
						<v-data-table
							class="dashboard_detail_table"
							height="130px"
							:headers="detailData.talk.table.headers"
							:items="detailData.talk.table.items"
							mobile-breakpoint="0"
							:disable-sort="false"
							:fixed-header="true"
							:hide-default-footer="true"
				...
				<!-- <v-flex xs12 class="dashboard_detail_main mt-3 pt-1">
					<v-layout class="my-2">
						...
						<v-flex xs3 class="mr-2">
							<select-box class="select_box_ico" :sel="detailData.orderGoods.select_1"></select-box>
						</v-flex>
						<v-flex xs3>
							<v-btn color="primary2" class="dash_business_btn" style="border-radius:5px;" block dark depressed height="25" width="69">
								적용
					...
					<v-layout class="mt-1 mb-2">
						<v-data-table
							class="dashboard_detail_table"
							height="130px"
							:headers="detailData.orderGoods.table.headers"
							:items="detailData.orderGoods.table.items"
							:disable-sort="false"
							:fixed-header="true"
							:hide-default-footer="true"
						>
				...
				<v-flex xs12 class="dashboard_detail_main mt-3 pt-1 mb-5">
					<v-layout align-center justify-center class="my-2">
						<v-flex xs3>
							<v-layout wrap align-center justify-center>
								<v-flex xs12 style="text-align:center; font-size:12px;">
									방문자
								</v-flex>
								<v-flex xs6 style="text-align:end; font-size:11px; color:#097CFF;">
									전체
								</v-flex>
								<v-flex xs6 style="text-align:center; font-size:16px; font-weight:bold;">
									131
								</v-flex>
								<v-flex xs6 style="text-align:end; font-size:11px; color:#FF7C09;">
									금일
								</v-flex>
								<v-flex xs6 style="text-align:center; font-size:16px; font-weight:bold;">
									14
			...
			<!-- 대시보드 컨텐츠 -->
			<v-layout v-else wrap class="dash_contents px-5">
				<!-- 사업지 정보 -->
...
export default {
			...
			detailData: {
				goodsCustomer: {
					...
					table: {
						...
						items: [
							{ dep1: '상담사 총 인원', dep2: '', goods: '-', customer: '' },
							{ dep1: '출근', dep2: '', goods: '-', customer: '' },
							{ dep1: '출근 미확인', dep2: '', goods: '-', customer: '' },
							{ dep1: '연차신청', dep2: '', goods: '-', customer: '' },
							{ dep1: '오늘 연차 및 반차 인원', dep2: '', goods: '-', customer: '' },
						],
					},
				},
				statusCustomer: {
					table: {
						...
						items: [
							{ db: '상담사 총 인원', dep2: '', goods: '-', customer: '' },
							{ db: '출근', dep2: '', goods: '-', customer: '' },
							{ db: '출근 미확인', dep2: '', goods: '-', customer: '' },
							{ db: '연차신청', dep2: '', goods: '-', customer: '' },
							{ db: '오늘 연차 및 반차 인원', dep2: '', goods: '-', customer: '' },
						],
					},
				},
				talk: {
	...
	methods: {
		click_move_work() {
			this.$router.push({ path: '/counselor/commuteManagement/manage' })
		},
		openInfo() {
		...
		go_to_manage(type) {
			if (type === '예약고객') {
				this.$router.push({ name: 'counselorBizReservation' })
			} else if (type === '등록고객') {
